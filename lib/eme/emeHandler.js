...
EMEHandler.prototype.init = function(video, licenseManager) {
  ...
  this.isSetServerCertificateSupported = false;
...
/** @return Promise */
EMEHandler.prototype.checkKeySystem = function(config) {
  ...
  this.checkKeySystem().then(function(keySystemAccess) {
    keySystemAccess.createMediaKeys().then(
      function(createdMediaKeys) {
        ...
        if (self.certificateSrc && createdMediaKeys.setServerCertificate) {
...
EMEHandler.prototype.setServerCertificate = function(mediaKeys, cert) {
  ...
  xhr.addEventListener('readystatechange', function(evt) {
    ...
    mediaKeys.setServerCertificate(evt.target.response).then(
      (result) => {
        if (result == true) {
          self.isSetServerCertificateSupported = true;
          dlog(2, 'SetServerCertificate returns true');
        }
      },
      (rejected) => {
        dlog(2, 'SetServerCertificate rejected');
      });
  });
