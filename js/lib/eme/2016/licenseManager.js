...
var LicenseManager = function(video, mediaStreams, flavor) {
  ...
  this.keys = [];
  this.kids = [];
  for (var i in this.mediaStreams) {
    this.keys.push(this.mediaStreams[i].get('key'));
 ...
 */
LicenseManager.prototype.acquireLicense = function(message, initData, cb) {
  if (LicenseManager.CLEARKEY == this.flavor) {
    ...
    cb(key, kid);
  } else if (LicenseManager.WIDEVINE == this.flavor ||
      LicenseManager.PLAYREADY == this.flavor) {
    this.requestLicense(message, function(key) {
      cb(key, initData);
    });
  }
};
