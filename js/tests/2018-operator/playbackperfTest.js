...
createFrameDropValidationTest(Media.H264.Webgl720p60fps, Media.H264.Webgl1080p60fps);
...
for (var formatIdx in allMediaFormats) {
  var videoStream = allMediaFormats[formatIdx];
  createPlaybackPerfTest(videoStream, 1.0, 'Playback Performance',
                         videoStream.codec == 'VP9');
}

createPlaybackPerfTest(Media.VP9.Webgl720p60fps, 0.25,
                       'Playback Rate Performance', true);
...
for (var formatIdx in mediaFormats) {
  var videoStream = mediaFormats[formatIdx];
  createPlaybackPerfTest(videoStream, 1.25, 'Playback Rate Performance',
                         videoStream.codec == 'VP9');
}

for (var formatIdx in mediaFormats) {
  var videoStream = mediaFormats[formatIdx];
  createPlaybackPerfTest(videoStream, 1.5, 'Playback Rate Performance',
                         videoStream.codec == 'VP9');
}

for (var formatIdx in mediaFormats) {
  var videoStream = mediaFormats[formatIdx];
  createPlaybackPerfTest(videoStream, 2.0, 'Playback Rate Performance',
                         videoStream.codec == 'VP9');
}
